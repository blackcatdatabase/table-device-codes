...
> Lineage: 0 outbound / 0 inbound · ✅ No engine drift detected · Index coverage: ready · PII flags: 6 · Changelog: fresh
...
| Pkg folder | [packages/device-codes](https://github.com/blackcatacademy/blackcat-database/blob/main/packages/device-codes) | Repo location |
...
- device_code_hash_key_version (key)
- device_code_key_version (key)
- id (key)
- token_payload (token)
- token_payload_key_version (token)
- user_code_hash_key_version (key)

## Usage
```bash
# Install/upgrade schema
pwsh -NoLogo -NoProfile -File scripts/schema-tools/Migrate-DryRun.ps1 -Package device-codes -Apply
