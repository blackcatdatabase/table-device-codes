...
namespace BlackCat\Database\Packages\DeviceCodes\Service;
 ...
 */
final class DeviceCodesAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private DeviceCodeRepository $deviceCodeRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'device_codes' => $this->deviceCodeRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\DeviceCodes\ModuleException("Record {$idStr} not found for locking.");
            }
