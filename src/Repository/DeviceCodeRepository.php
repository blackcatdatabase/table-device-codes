...
use BlackCat\Database\Packages\DeviceCodes\Definitions;
use BlackCat\Database\Packages\DeviceCodes\Criteria;
use BlackCat\Database\Packages\DeviceCodes\Dto\DeviceCodeDto as Dto;
use BlackCat\Database\Packages\DeviceCodes\Mapper\DeviceCodeDtoMapper as RowMapper;
...
use BlackCat\Database\Packages\DeviceCodes\Repository\DeviceCodeRepositoryInterface;
...
class DeviceCodeRepository implements DeviceCodeRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'device_code_hash' ];
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\DeviceCodes\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'token_payload', 'token_payload_key_version', 'approved_at' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\DeviceCodes\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'token_payload', 'token_payload_key_version', 'approved_at' ]);
          return count($rows);
        }
    ...
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeviceCodes\Dto\DeviceCodeDto|null */
    public function getByDeviceCodeHash(string $deviceCodeHash, bool $asDto = false): array|\BlackCat\Database\Packages\DeviceCodes\Dto\DeviceCodeDto|null {
        return $this->getByUnique([ 'device_code_hash' => $deviceCodeHash ], $asDto);
    }
    public function existsByDeviceCodeHash(string $deviceCodeHash): bool {
        $where = 't.' . Ident::q($this->db, 'device_code_hash') . ' = :uniq_device_code_hash';
        return $this->exists($where, [ 'uniq_device_code_hash' => $deviceCodeHash ]);
    }
    /** @return int|string|null */
    public function getIdByDeviceCodeHash(string $deviceCodeHash) {
        $row = $this->getByDeviceCodeHash($deviceCodeHash, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
    /** @return array<string,mixed>|\BlackCat\Database\Packages\DeviceCodes\Dto\DeviceCodeDto|null */
    public function getByUserCodeHash(string $userCodeHash, bool $asDto = false): array|\BlackCat\Database\Packages\DeviceCodes\Dto\DeviceCodeDto|null {
        return $this->getByUnique([ 'user_code_hash' => $userCodeHash ], $asDto);
    }
    public function existsByUserCodeHash(string $userCodeHash): bool {
        $where = 't.' . Ident::q($this->db, 'user_code_hash') . ' = :uniq_user_code_hash';
        return $this->exists($where, [ 'uniq_user_code_hash' => $userCodeHash ]);
    }
    /** @return int|string|null */
    public function getIdByUserCodeHash(string $userCodeHash) {
        $row = $this->getByUserCodeHash($userCodeHash, false);
        if (!is_array($row)) { return null; }
        return $row['id'] ?? null;
    }
